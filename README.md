...
### v3.1（工程化优化，参考社区同类项目最佳实践）
- **单实例保护**：命名互斥量防止双开（双图标 + HID 句柄争用）
- **滚动日志**：`%LOCALAPPDATA%\MCHOSEBattery\logs\app.log`（1MB×2，设 `MCHOSE_DEBUG=1` 开 DEBUG），
  设备开关/电量推送/异常全部留痕，出问题不再靠猜
- **监听线程加固**：键盘推送监听线程任何未知异常都会在 3 秒后自动重启，不会静默死亡
- **双设备低电量通知**：键盘 ≤20% 也会弹提醒（原来只有鼠标），回到 25% 以上重新布防
- **协议解析可测试**：纯函数抽取（鼠标帧/FA FB 07/MANUAL_REPORT/设备名匹配），
  `python -m unittest discover tests` 覆盖 11 个用例
- **图标去重重绘**：数据未变化时跳过 ICO 重编码，降低常驻开销
- 补 `requirements.txt`；`--version` 查询版本；清理尾部重复代码块

### v3.0
- **蓝牙模式全面修复**（真机实测双设备可读）：
  - 修正 BLE 实例 ID 正则（PID 与 REV 段之间漏了 `_` 分隔符，导致蓝牙探测永远为空）；
  - PowerShell 输出改按 ANSI 解码（UTF-8 解码线程遇非 ASCII 字节崩溃，蓝牙读数静默丢失）；
  - 蓝牙闲置断链兜底：BLE 链路省电断开时（HID 集合消失），24 小时内连接过则使用 GATT 电量缓存。
- **读取优先级重构**：蓝牙硬在线（实时）> 各自 2.4G 通道 > 蓝牙闲置缓存，模式切换互不覆盖。
- **模式切换提速**：监听器听到接收器链路建立/断开广播（`FA FB 06`）时立即作废蓝牙扫描缓存，
  键盘切模式后 5~10 秒内反映新模式（原最长 60 秒），与鼠标持平。
- 悬停提示精简：移除推送时间标注。
